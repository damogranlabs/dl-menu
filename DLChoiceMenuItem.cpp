...
/// ChoiceMenuItem
///
DLChoiceMenuItem::DLChoiceMenuItem(LiquidCrystal *lcd, char *label,	int address, const char **c, int dval) : DLMenuItem(lcd, label, address){
  ...
  // load from EEPROM
  if((long)EEPROM[address] == EEPROM_NULL) selected = dval;
  else selected = EEPROM[address];
