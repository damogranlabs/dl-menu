...
/// FloatMenuItem
///
DLIntMenuItem::DLIntMenuItem(LiquidCrystal *lcd, char *label, int address, int d, int dval) : DLMenuItem(lcd, label, address){
  // prepare stuff
  n = new DLNumber(address);
  if(n->getUintValue() == EEPROM_NULL) n->setValue((unsigned long)dval);
