...
bool DLMenu::check(void){
  ...
  if(!(bl || br || bu || bd)){
    if(index == -1) return false; // there's nothing going on anyway
    else if(millis() - tTouched > M_TIMEOUT) return quit(); // timeout! quit
    else return true;
  }
  else{
    ...
    if(index == -1){
      ...
      if(bl){
        ...
        if(!items[index]->previous()){
          ...
          if(index == -1) return quit();
          else items[index]->show(true);
        }
      }
      else if(br){
        if(!items[index]->next()){
